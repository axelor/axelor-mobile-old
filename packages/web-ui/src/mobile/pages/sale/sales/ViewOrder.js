...
import { TabberView, CardView, SwiperView } from "../../../components";
...
class ViewOrder extends PageCommentList {
  constructor(props) {
    super(props);
        ...
        >
          ...
          >
            {!loading
              ? order.id !== undefined
                ? `${translate(
                    "common.saleOrderHeader"
                  )} ${order.sale_Order_Seq || order.saleOrderSeq || ""}`
                : translate("common.newOrder")
              : ""}
          </span>
        </div>
          ...
          <PageElement key="delete" offline={!order.id}>
            {this.state.edit ? (
              ...
              <div
                ...
                className="round-icon"
                onClick={() => this.onDeleteSaleOrder(order)}
              >
                <Icon icon="fa-trash" />
              </div>
            )}
  ...
  onSave(e) {
    ...
    this.setState({ isLoading: true });
    let { orderForm, recordList } = this.state;
    const { update, add } = this.props.order;
    const { updateOrder, addOrder } = this.props.route;
    const { info } = this.props.user_data.data;
  ...
  onDeleteSaleOrder(order) {
    const { remove } = this.props.order;
    ons.notification
      .confirm(translate("Alert.confirmDelete"), {
        title: translate("Alert.confirm"),
        id: "delete-sale-order"
      })
      .then(res => {
        if (res === 1) {
          remove(order).then(res => {
            if (this.props.route.removeOrder) {
              this.props.route.removeOrder(order);
            }
            this.props.navigator.popPage();
          });
        }
    ...
    return (
      <div style={{ marginBottom: 25 }} className="common-font-size">
          ...
          {this.props.app.mode === "online" && !this.state.isNew && (
            <CardView.ActionView>
              <CardView.ActionItem
                ...
                iconClassName="order-action-icon"
                icon="fa-eye"
            ...
            <div className="product-total">
              ...
              <div className="product-total-text common-font-size">
                {getSepratedPrice(
                  ...
                  `${
                    orderForm.currency.symbol ? orderForm.currency.symbol : "€"
                  }`}
              </div>
            </div>
        ...
        {this.state.activeTab === 2 ? (
          sale_order_line_tax_list && sale_order_line_tax_list.length > 0 ? (
            <div>
              <div
                style={{
                  display: "flex",
              ...
              <div>
                {sale_order_line_tax_list.map((c, i) => (
                  <div
                    key={i}
                    style={{ display: "flex", marginTop: 10, paddingLeft: 21 }}
                    className="sale-product-list"
                  >
                    <div
                    ...
                    >
                      {getSepratedPrice(
                        parseFloat(c.exTaxBase || c.baseWT || "0.00").toFixed(2)
                      )}
                    </span>
                    <span
                      style={{ flex: 1, textAlign: "left", fontSize: "9pt" }}
          ...
          <div>
            <CardView>
              ...
              <RenderManyToOne
                ...
                liveSearch={false}
                searchAPI={e => this.props.duration.search(e)}
                onChange={e =>
              ...
              <RenderManyToOne
                ...
                liveSearch={false}
                searchAPI={e => this.props.paymentmode.search(e)}
...
ViewOrder = reduxConnect(mapPropsToState)(ViewOrder);
...
export default connect(mapConnectToProps)(ViewOrder, mapConnectConfig);
